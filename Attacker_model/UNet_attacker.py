...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, y=None, lq=None):
        ...
        hs = []
        #timesteps = torch.tensor([timesteps], device=device, dtype=torch.float32)  # Make it 1D
        #print("timesteps:", timesteps)  # Debug
        #print("timesteps input shape:", timesteps.shape, timesteps.dtype, timesteps)
        #print("timestep_embedding output shape:", timestep_embedding(timesteps, self.model_channels).shape)
        #print("model_channels:", self.model_channels)  # Debug
        emb = self.time_embed(timestep_embedding(timesteps.to(torch.long), self.model_channels)).type(self.dtype)
        #emb = self.time_embed(timestep_embedding(timesteps, self.model_channels)).type(self.dtype)
        #print("timestep_embedding output shape:", emb.shape)  # Debug
        if self.num_classes is not None:
