...
from utils_SplitFedDiffuse_V3 import (get_loaders, eval, get_loaders_test, test)
...
def train(train_loader, local_model1, local_model2, local_model3, denoiser1, denoiser2, global_dn_pred1,
    ...
    def visualize_features(features, title, ncols=3):
        num_samples = min(9, features.shape[0])  # Display at most 9 images
    ...
    for batch_idx, (data, targets) in enumerate(loop):
        ...
        # ViT_out = ViT_out.unsqueeze(-1).unsqueeze(-1)
        # ViT_out = F.interpolate(ViT_out, size=(112, 112), mode="bilinear", align_corners=False)
        # ViT_out_noisy = forward_diffusion(ViT_out,beta,alpha_cum, t)
        # ViT_out_noisy = ViT_out_noisy[:, 0:1, :, :]  # Keep only 1 channel
        # ViT_out_noisy_resized = reshapenet(ViT_out_noisy)
        ...
        # combined_pred1 = attention_weights1 * normalize_features(predictions1) + (1 - attention_weights1) * normalize_features(ViT_out_noisy_resized)
        # combined_pred1 = ViT_out_noisy_resized + predictions1
        ...
        # predictions2_noisy = forward_diffusion(predictions2,beta,alpha_cum, t)
        # attention_weights3 = torch.sigmoid(torch.mean(predictions2, dim=(2,3), keepdim=True))
        # combined_pred2 = attention_weights3 * normalize_features(predictions2) + (1 - attention_weights3) * normalize_features(ViT_out_noisy_resized)
    ...
    # all_predictions1_noisy = torch.cat(all_predictions1_noisy, dim=0)
    ...
    # visualize_features(all_predictions1, "all_predictions1")
    # visualize_features(all_predictions1_noisy, "all_predictions1_noisy")
    # visualize_features(all_ViT_out_noisy_resized, "all_ViT_out_noisy_resized")
    # visualize_features(all_combined_pred1, "all_combinedpred1_noisy")
...
def main():
    ...
    # weights_path = "/local-scratch/localhome/csj5/compressenv/weights_path/vit_b_16-c867db91.pth"
    # state_dict = torch.load(weights_path)
    # global_ViTmodel.load_state_dict(state_dict)
    # print(global_ViTmodel)
    # global_ViTmodel.load_state_dict(torch.load('/local-scratch/localhome/csj5/compressenv/weights_path/globalViT.pth'))
    ...
    test_epoch_loss, test_epoch_seg_loss, test_epoch_recon1_loss, test_epoch_recon2_loss, test_epoch_acc, test_epoch_accwithback, test_epoch_accnoback, global_dn_pred1, global_dn_pred2 = test(
        test_loader, global_model1_fed, global_model2_fed, global_model3_fed, global_denoiser1, global_denoiser2,
        loss_fn, test_folder)
    ...
    for com_round in (range(args.rounds)):
        local_weights1, local_weights2, local_weights3, den1_weights, den2_weights = [], [], [], [], []
            ...
            for epoch in range(args.local_ep):
                ...
                if cl_idx == 1:
                    train_epoch_loss, train_epoch_acc, trainepoch_iou_withbackground, trainepoch_iou_nobackground, trainepoch_iou_class0, trainepoch_iou_class1, trainepoch_iou_class2 = train(
                        train_loader, local_model1, local_model2, local_model3, local_denoiser1, local_denoiser2,
                        global_dn_pred1, global_dn_pred2, optimizer1, optimizer2, optimizer3, loss_fn)
                    ...
                    val_epoch_loss, val_epoch_acc, valepoch_iou_withbackground, valepoch_iou_nobackground, valepoch_iou_class0, valepoch_iou_class1, valepoch_iou_class2 = eval(
                        val_loader, local_model1, local_model2, local_model3, local_denoiser1, local_denoiser2,
                        global_dn_pred1, global_dn_pred2, loss_fn)
                    ...
                    if least_lossC1 > val_epoch_loss:
                        ...
                        torch.save(local_model1.state_dict(),
                                   './Saved/Checkpoints/C1M1_localcheckpoint.pth')
                        torch.save(local_model2.state_dict(),
                                   './Saved/Checkpoints/C1M2_localcheckpoint.pth')
                        torch.save(local_model3.state_dict(),
                                   './Saved/Checkpoints/C1M3_localcheckpoint.pth')
                        torch.save(local_denoiser1.state_dict(),
                                   './Saved/Checkpoints/C1_denoiser1.pth')
                        torch.save(local_denoiser2.state_dict(),
                                   './Saved/Checkpoints/C1_denoiser2.pth')
                ...
                if cl_idx == 2:
                    ...
                    if least_lossC2 > val_epoch_loss:
                        ...
                        torch.save(local_model1.state_dict(),
                                   './Saved/Checkpoints/C2M1_localcheckpoint.pth')
                        torch.save(local_model2.state_dict(),
                                   './Saved/Checkpoints/C2M2_localcheckpoint.pth')
                        torch.save(local_model3.state_dict(),
                                   './Saved/Checkpoints/C2M3_localcheckpoint.pth')
                        torch.save(local_denoiser1.state_dict(),
                                   './Saved/Checkpoints/C2_denoiser1.pth')
                        torch.save(local_denoiser2.state_dict(),
                                   './Saved/Checkpoints/C2_denoiser2.pth')
                ...
                if cl_idx == 3:
                    ...
                    if least_lossC3 > val_epoch_loss:
                        ...
                        torch.save(local_model1.state_dict(),
                                   './Saved/Checkpoints/C3M1_localcheckpoint.pth')
                        torch.save(local_model2.state_dict(),
                                   './Saved/Checkpoints/C3M2_localcheckpoint.pth')
                        torch.save(local_model3.state_dict(),
                                   './Saved/Checkpoints/C3M3_localcheckpoint.pth')
                        torch.save(local_denoiser1.state_dict(),
                                   './Saved/Checkpoints/C3_denoiser1.pth')
                        torch.save(local_denoiser2.state_dict(),
                                   './Saved/Checkpoints/C3_denoiser2.pth')
                ...
                if cl_idx == 4:
                    ...
                    if least_lossC4 > val_epoch_loss:
                        ...
                        torch.save(local_model1.state_dict(),
                                   './Saved/Checkpoints/C4M1_localcheckpoint.pth')
                        torch.save(local_model2.state_dict(),
                                   './Saved/Checkpoints/C4M2_localcheckpoint.pth')
                        torch.save(local_model3.state_dict(),
                                   './Saved/Checkpoints/C4M3_localcheckpoint.pth')
                        torch.save(local_denoiser1.state_dict(),
                                   './Saved/Checkpoints/C4_denoiser1.pth')
                        torch.save(local_denoiser2.state_dict(),
                                   './Saved/Checkpoints/C4_denoiser2.pth')
                ...
                if cl_idx == 5:
                    ...
                    if least_lossC5 > val_epoch_loss:
                        ...
                        torch.save(local_model1.state_dict(),
                                   './Saved/Checkpoints/C5M1_localcheckpoint.pth')
                        torch.save(local_model2.state_dict(),
                                   './Saved/Checkpoints/C5M2_localcheckpoint.pth')
                        torch.save(local_model3.state_dict(),
                                   './Saved/Checkpoints/C5M3_localcheckpoint.pth')
                        torch.save(local_denoiser1.state_dict(),
                                   './Saved/Checkpoints/C5_denoiser1.pth')
                        torch.save(local_denoiser2.state_dict(),
                                   './Saved/Checkpoints/C5_denoiser2.pth')
            ...
            for epoch in range(args.val_global_ep):
                ...
                if cl_idx == 1:
                    g_val_epoch_loss, g_val_epoch_acc, g_val_epoch_iouwithback, g_val_epoch_iounoback, g_valepoch_iou_class0, g_valepoch_iou_class1, g_valepoch_iou_class2 = eval(
                        val_loader, global_model1_fed, global_model2_fed, global_model3_fed, global_denoiser1,
                        global_denoiser2, global_dn_pred1, global_dn_pred2, loss_fn)
