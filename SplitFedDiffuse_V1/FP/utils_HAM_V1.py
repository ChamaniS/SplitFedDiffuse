...
def eval(loader, local_model1,local_model2,local_model3,local_ViTmodel,reshapenet,vdenoiser1,vdenoiser2,global_dn_pred1,global_dn_pred2,loss_fn):
    ...
    with torch.no_grad():
        for idx, (x, y) in enumerate(loader):
            ...
            ViT_out_noisy_resized = F.interpolate(ViT_out_resized, size=(112, 112), mode="bilinear",
                                                  align_corners=False)
            ...
            #-----------to prevent indexing error-----------
            unique_classes = np.unique(preds.cpu().numpy())
            if 1 not in unique_classes:
                iou_sklearn = np.array([jaccard_score(y.cpu().flatten(), preds.cpu().flatten(), average=None)[0], 0.0])
            else:
                iou_sklearn = jaccard_score(y.cpu().flatten(), preds.cpu().flatten(), average=None)
...
def test(loader, modelclientFE,modelserver,modelclientBE,test_ViTmodel,reshapenet,tdenoiser1,tdenoiser2, loss_fn,folder):
    ...
    with torch.no_grad():
        for idx,(x, y) in enumerate(loader):
            ...
            combined_pred2 = attention_weights2 * normalize_features(predictions2) + (
                    1 - attention_weights2) * normalize_features(ViT_out_noisy_resized)
    ...
    #torch.save(alldenoised_pred1, '/local-scratch/localhome/csj5/compressenv/allnoisy_pred1_HAM_V1.pth')
