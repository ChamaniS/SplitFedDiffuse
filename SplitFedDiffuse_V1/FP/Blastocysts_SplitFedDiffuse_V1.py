...
# 1. Train function
def train(train_loader, local_model1, local_model2, local_model3, local_ViTmodel, reshapenet, denoiser1, denoiser2,
    ...
    def calculate_fid(tensor1, tensor2):
        ...
        # Reshape from (batch, channels, height, width) ? (batch, features)
        tensor1_np = tensor1_np.reshape(tensor1_np.shape[0], -1)  # Flatten each sample
        tensor2_np = tensor2_np.reshape(tensor2_np.shape[0], -1)  # Flatten each sample

        # Compute mean
        mu1, mu2 = tensor1_np.mean(axis=0), tensor2_np.mean(axis=0)

        # Compute covariance matrices
        if tensor1_np.shape[0] > 1:  # Only compute if batch size > 1
            sigma1 = np.cov(tensor1_np, rowvar=False)
            sigma2 = np.cov(tensor2_np, rowvar=False)
        else:
            # If batch size = 1, use variance instead
        ...
        except ValueError:  # Handle case where covmean computation fails
    ...
    train_running_mseS2_V3 = 0.0
    train_running_SD2_V3 = 0.0
    train_running_mseS2_V1 = 0.0
    train_running_SD2_V1 = 0.0
    train_running_ssim_V1 = 0.0
    train_running_ssim_V3 = 0.0
    train_running_psnr_V1 = 0.0
    train_running_psnr_V3 = 0.0
    train_running_fid_V1 = 0.0
    train_running_fid_V3 = 0.0
    # all_pred1 =[]
    all_pred2_baseline = []
    all_noisy_pred1 = []
    vit_noisy = []
    ...
    for batch_idx, (data, targets) in enumerate(loop):
        ...
        predictions2_baseline = local_model2(predictions1)
        ...
        combined_pred1 = normalize_features(predictions1) + normalize_features(ViT_out_noisy)
        ...
        combined_pred2 = normalize_features(predictions2) + normalize_features(ViT_out_noisy)
    ...
    # all_pred1= torch.cat(all_pred1, dim=0)
    # visualize_features(all_pred1, "predictions1")
    #all_pred2_baseline = torch.cat(all_pred2_baseline, dim=0)
    #visualize_features(all_pred2_baseline, "all_pred2_baseline")
    #all_agg = torch.cat(all_agg, dim=0)
    #visualize_features(all_agg, "all_agg_split2")
    # all_noisy_pred1= torch.cat(all_noisy_pred1, dim=0)
    # visualize_features(all_noisy_pred1, "noisy_pred1")
    # vit_noisy= torch.cat(vit_noisy, dim=0)
    # visualize_features(vit_noisy, "vit_noisy")
    # before_denoise= torch.cat(before_denoise, dim=0)
    # visualize_features(before_denoise, "before_denoise")
    # after_denoise= torch.cat(after_denoise, dim=0)
    # visualize_features(after_denoise, "after_denoise")
            ...
            for epoch in range(args.local_ep):
                ...
                if cl_idx == 1:
                    train_epoch_loss, train_epoch_acc, trainepoch_iou_withbackground, trainepoch_iou_nobackground, trainepoch_iou_class0, trainepoch_iou_class1, trainepoch_iou_class2, trainepoch_iou_class3, trainepoch_iou_class4 = train(
                        train_loader, local_model1, local_model2, local_model3, local_ViTmodel, local_reshapenet,
                        local_denoiser1, local_denoiser2, global_dn_pred1, global_dn_pred2, optimizer1, optimizer2,
                        optimizer3, loss_fn)
                    ...
                    val_epoch_loss, val_epoch_acc, valepoch_iou_withbackground, valepoch_iou_nobackground, valepoch_iou_class0, valepoch_iou_class1, valepoch_iou_class2, valepoch_iou_class3, valepoch_iou_class4 = eval(
                        ...
                        local_denoiser1, local_denoiser2, global_dn_pred1, global_dn_pred2, loss_fn)
                    ...
                    if least_lossC1 > val_epoch_loss:
                        ...
                        torch.save(local_ViTmodel.state_dict(), save_F + '/Checkpoints/C1_local_ViTmodel.pth')
                        torch.save(local_reshapenet.state_dict(), save_F + '/Checkpoints/C1_local_reshapenet.pth')
                ...
                if cl_idx == 2:
                    ...
                    if least_lossC2 > val_epoch_loss:
                        ...
                        torch.save(local_ViTmodel.state_dict(), save_F + '/Checkpoints/C2_local_ViTmodel.pth')
                        torch.save(local_reshapenet.state_dict(), save_F + '/Checkpoints/C2_local_reshapenet.pth')
                ...
                if cl_idx == 3:
                    ...
                    if least_lossC3 > val_epoch_loss:
                        ...
                        torch.save(local_ViTmodel.state_dict(), save_F + '/Checkpoints/C3_local_ViTmodel.pth')
                        torch.save(local_reshapenet.state_dict(), save_F + '/Checkpoints/C3_local_reshapenet.pth')
                ...
                if cl_idx == 4:
                    ...
                    if least_lossC4 > val_epoch_loss:
                        ...
                        torch.save(local_ViTmodel.state_dict(), save_F + '/Checkpoints/C4_local_ViTmodel.pth')
                        torch.save(local_reshapenet.state_dict(), save_F + '/Checkpoints/C4_local_reshapenet.pth')
                ...
                if cl_idx == 5:
                    ...
                    if least_lossC5 > val_epoch_loss:
                        ...
                        torch.save(local_ViTmodel.state_dict(), save_F + '/Checkpoints/C5_local_ViTmodel.pth')
                        torch.save(local_reshapenet.state_dict(), save_F + '/Checkpoints/C5_local_reshapenet.pth')
