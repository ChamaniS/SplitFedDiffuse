...
from utils_HAM_FBP_V1 import (get_loaders_test, eval, get_loaders_test, test)
...
DataF = "C:/Users/csj5/Projects/Data/HAM10000/HAM10000/datafed/highly_nonuniform/"
save_F = "C:/Users/csj5/Projects/Splitfed_V333_DD/Saved"

# client 1
TRAIN_IMG_DIR_C1 = DataF + "/client5/train_imgs/"
TRAIN_MASK_DIR_C1 = DataF + "/client5/train_masks/"
VAL_IMG_DIR_C1 = DataF + "/client5/val_imgs/"
VAL_MASK_DIR_C1 = DataF + "/client5/val_masks/"

# client 2
TRAIN_IMG_DIR_C2 = DataF + "/client5/train_imgs/"
TRAIN_MASK_DIR_C2 = DataF + "/client5/train_masks/"
VAL_IMG_DIR_C2 = DataF + "/client5/val_imgs/"
VAL_MASK_DIR_C2 = DataF + "/client5/val_masks/"

# client 3
TRAIN_IMG_DIR_C3 = DataF + "/client5/train_imgs/"
TRAIN_MASK_DIR_C3 = DataF + "/client5/train_masks/"
VAL_IMG_DIR_C3 = DataF + "/client5/val_imgs/"
VAL_MASK_DIR_C3 = DataF + "/client5/val_masks/"

# client 4
TRAIN_IMG_DIR_C4 = DataF + "/client5/train_imgs/"
TRAIN_MASK_DIR_C4 = DataF + "/client5/train_masks/"
VAL_IMG_DIR_C4 = DataF + "/client5/val_imgs/"
VAL_MASK_DIR_C4 = DataF + "/client5/val_masks/"
...
TEST_IMG_DIR = DataF + "/client5/val_imgs/"
TEST_MASK_DIR = DataF + "/client5/val_masks/"
...
# 1. Train function
def train(train_loader, local_model1, local_model2, local_model3, local_ViTmodel, reshapenet, reshapenet_g,denoiser1, denoiser2,
          denoiser3, denoiser4, global_dn_pred1, global_dn_pred2, optimizer1, optimizer2, optimizer3, loss_fn):
    ...
    for batch_idx, (data, targets) in enumerate(loop):
        ...
        combined_pred1 = attention_weights1 * normalize_features(predictions1) + (1 - attention_weights1) * normalize_features(ViT_out_noisy)
        ...
        combined_pred2 = attention_weights3 * normalize_features(predictions2) + (1 - attention_weights3) * normalize_features(ViT_out_noisy)
        ...
        mygrad3 = grads3
        ViT_out_g = local_ViTmodel(data)

        ViT_out_g = ViT_out_g.detach()
        ViT_out_g = ViT_out_g[:, :1, None, None]
        ViT_out_g_reshaped = reshapenet_g(ViT_out_g)
        ViT_out_g_resized = F.interpolate(ViT_out_g_reshaped, size=(112, 112), mode="bilinear", align_corners=False)
        ViT_out_g_noisy = forward_diffusion(ViT_out_g_resized, beta, alpha_cum, t)
        attention_weights5 = torch.sigmoid(torch.mean(mygrad3, dim=(2, 3), keepdim=True))
        combined_grad3 = attention_weights5 * normalize_features(mygrad3) + (1 - attention_weights5) * normalize_features(ViT_out_g_noisy)
        # Denoising model 3
        denoised_pred3 = denoise(normalize_features(combined_grad3), denoiser3, alpha_cum, t, device)
        ...
        attention_weights6 = torch.sigmoid(torch.mean(mygrad2, dim=(2, 3), keepdim=True))
        combined_grad4 = attention_weights6 * normalize_features(mygrad2) + (1 - attention_weights6) * normalize_features(ViT_out_g_noisy)
        # Denoising model 3
        denoised_pred4 = denoise(normalize_features(combined_grad4), denoiser4, alpha_cum, t, device)
        recon_loss4 = F.mse_loss(normalize_gradients(denoised_pred4),normalize_gradients(mygrad2))  # reconstruction loss of LDDM-3
...
def main():
    ...
    train_loader_C1, val_loader_C1 = get_loaders_test(
        TRAIN_IMG_DIR_C1,
        TRAIN_MASK_DIR_C1,
        VAL_IMG_DIR_C1,
        VAL_MASK_DIR_C1,
    ...
    train_loader_C2, val_loader_C2 = get_loaders_test(
        TRAIN_IMG_DIR_C2,
        TRAIN_MASK_DIR_C2,
        VAL_IMG_DIR_C2,
        VAL_MASK_DIR_C2,
    ...
    train_loader_C3, val_loader_C3 = get_loaders_test(
        TRAIN_IMG_DIR_C3,
        TRAIN_MASK_DIR_C3,
        VAL_IMG_DIR_C3,
        VAL_MASK_DIR_C3,
    ...
    train_loader_C4, val_loader_C4 = get_loaders_test(
        TRAIN_IMG_DIR_C4,
        TRAIN_MASK_DIR_C4,
        VAL_IMG_DIR_C4,
        VAL_MASK_DIR_C4,
    ...
    train_loader_C5, val_loader_C5 = get_loaders_test(
        TRAIN_IMG_DIR_C5,
        TRAIN_MASK_DIR_C5,
        VAL_IMG_DIR_C5,
        VAL_MASK_DIR_C5,
    ...
    # Load the pretrained ViT model with weights
    # weights = ViT_B_16_Weights.IMAGENET1K_V1
    # global_ViTmodel = vit_b_16(weights=weights)
    # weights_path = "/project/6008975/csj5/myenv/weights_path/vit_b_16-c867db91.pth"
    global_ViTmodel =timm.create_model("vit_base_patch16_224", pretrained=True).to(device)
    # state_dict = torch.load(weights_path)
    # global_ViTmodel.load_state_dict(state_dict)
    # print(global_ViTmodel)
    ...
    global_reshapenet_g = reshapenet(in_channels=1).to(device)
    # global_model1_fed.load_state_dict(torch.load(save_F + './Checkpoints/M1_globalcheckpoint.pth'))
    # global_model2_fed.load_state_dict(torch.load(save_F + './Checkpoints/M2_globalcheckpoint.pth'))
    # global_model3_fed.load_state_dict(torch.load(save_F + './Checkpoints/M3_globalcheckpoint.pth'))
    # global_ViTmodel.load_state_dict(torch.load(save_F + './Checkpoints/globalViT.pth'))
    # global_reshapenet.load_state_dict(torch.load(save_F + './Checkpoints/global_reshapenet.pth'))
    # global_denoiser1.load_state_dict(torch.load(save_F + './Checkpoints/global_denoiser1.pth'))
    # global_denoiser2.load_state_dict(torch.load(save_F + './Checkpoints/global_denoiser2.pth'))
    ...
    test_epoch_loss, test_epoch_seg_loss, test_epoch_recon1_loss, test_epoch_recon2_loss, test_epoch_acc, test_epoch_accwithback, test_epoch_accnoback, global_dn_pred1, global_dn_pred2 = test(
        test_loader, global_model1_fed, global_model2_fed, global_model3_fed, global_ViTmodel, global_reshapenet,
        global_denoiser1, global_denoiser2, loss_fn, test_folder)
    # print("global_dn_pred1",global_dn_pred1)
    # print("global_dn_pred2",global_dn_pred2)
    ...
    for com_round in (range(args.rounds)):
        local_weights1, local_weights2, local_weights3, ViT_weights, den1_weights, den2_weights, den3_weights, den4_weights, reshape_weights,reshape_g_weights = [], [], [], [], [], [], [], [], [],[]
        ...
        for idx in idxs_users:
            ...
            local_reshapenet_g = copy.deepcopy(global_reshapenet_g)
            ...
            for epoch in range(args.local_ep):
                ...
                if cl_idx == 1:
                    train_epoch_loss, train_epoch_acc, trainepoch_iou_withbackground, trainepoch_iou_nobackground, trainepoch_iou_class0, trainepoch_iou_class1 = train(
                        train_loader, local_model1, local_model2, local_model3, local_ViTmodel, local_reshapenet,local_reshapenet_g,local_denoiser1, local_denoiser2, local_denoiser3, local_denoiser4, global_dn_pred1,global_dn_pred2, optimizer1, optimizer2, optimizer3, loss_fn)
                    ...
                    val_epoch_loss, val_epoch_acc, valepoch_iou_withbackground, valepoch_iou_nobackground, valepoch_iou_class0, valepoch_iou_class1 = eval(
                        val_loader, local_model1, local_model2, local_model3, local_ViTmodel, local_reshapenet,
                        local_denoiser1, local_denoiser2, global_dn_pred1, global_dn_pred2, loss_fn, folder)
                    ...
                    if least_lossC1 > val_epoch_loss:
                        ...
                        torch.save(local_model1.state_dict(),
                                   save_F + './Checkpoints/C1M1_localcheckpoint.pth')
                        torch.save(local_model2.state_dict(),
                                   save_F + './Checkpoints/C1M2_localcheckpoint.pth')
                        torch.save(local_model3.state_dict(),
                                   save_F + './Checkpoints/C1M3_localcheckpoint.pth')
                        torch.save(local_ViTmodel.state_dict(),
                                   save_F + './Checkpoints/C1_local_ViTmodel.pth')
                        torch.save(local_denoiser1.state_dict(),
                                   save_F + './Checkpoints/C1_denoiser1.pth')
                        torch.save(local_denoiser2.state_dict(),
                                   save_F + './Checkpoints/C1_denoiser2.pth')
                        torch.save(local_denoiser3.state_dict(),
                                   save_F + './Checkpoints/C1_denoiser3.pth')
                        torch.save(local_denoiser4.state_dict(),
                                   save_F + './Checkpoints/C1_denoiser4.pth')
                        torch.save(local_reshapenet.state_dict(),
                                   save_F + './Checkpoints/C1_local_reshapenet.pth')
                        torch.save(local_reshapenet_g.state_dict(),save_F + './Checkpoints/C1_local_reshapenet_g.pth')
                        print('C1localmodel saved')
                if cl_idx == 2:
                    train_epoch_loss, train_epoch_acc, trainepoch_iou_withbackground, trainepoch_iou_nobackground, trainepoch_iou_class0, trainepoch_iou_class1 = train(
                        train_loader, local_model1, local_model2, local_model3, local_ViTmodel, local_reshapenet,local_reshapenet_g,
                        local_denoiser1, local_denoiser2, local_denoiser3, local_denoiser4, global_dn_pred1,
                    ...
                    if least_lossC2 > val_epoch_loss:
                        ...
                        torch.save(local_model1.state_dict(),
                                   save_F + './Checkpoints/C2M1_localcheckpoint.pth')
                        torch.save(local_model2.state_dict(),
                                   save_F + './Checkpoints/C2M2_localcheckpoint.pth')
                        torch.save(local_model3.state_dict(),
                                   save_F + './Checkpoints/C2M3_localcheckpoint.pth')
                        torch.save(local_ViTmodel.state_dict(),
                                   save_F + './Checkpoints/C2_local_ViTmodel.pth')
                        torch.save(local_denoiser1.state_dict(),
                                   save_F + './Checkpoints/C2_denoiser1.pth')
                        torch.save(local_denoiser2.state_dict(),
                                   save_F + './Checkpoints/C2_denoiser2.pth')
                        torch.save(local_denoiser3.state_dict(),
                                   save_F + './Checkpoints/C2_denoiser3.pth')
                        torch.save(local_denoiser4.state_dict(),
                                   save_F + './Checkpoints/C2_denoiser4.pth')
                        torch.save(local_reshapenet.state_dict(),
                                   save_F + './Checkpoints/C2_local_reshapenet.pth')
                        torch.save(local_reshapenet_g.state_dict(),save_F + './Checkpoints/C2_local_reshapenet_g.pth')
                ...
                if cl_idx == 3:
                    ...
                    if least_lossC3 > val_epoch_loss:
                        ...
                        torch.save(local_model1.state_dict(),
                                   save_F + './Checkpoints/C3M1_localcheckpoint.pth')
                        torch.save(local_model2.state_dict(),
                                   save_F + './Checkpoints/C3M2_localcheckpoint.pth')
                        torch.save(local_model3.state_dict(),
                                   save_F + './Checkpoints/C3M3_localcheckpoint.pth')
                        torch.save(local_ViTmodel.state_dict(),
                                   save_F + './Checkpoints/C3_local_ViTmodel.pth')
                        torch.save(local_denoiser1.state_dict(),
                                   save_F + './Checkpoints/C3_denoiser1.pth')
                        torch.save(local_denoiser2.state_dict(),
                                   save_F + './Checkpoints/C3_denoiser2.pth')
                        torch.save(local_denoiser3.state_dict(),
                                   save_F + './Checkpoints/C3_denoiser3.pth')
                        torch.save(local_denoiser4.state_dict(),
                                   save_F + './Checkpoints/C3_denoiser4.pth')
                        torch.save(local_reshapenet.state_dict(),
                                   save_F + './Checkpoints/C3_local_reshapenet.pth')
                        torch.save(local_reshapenet_g.state_dict(),save_F + './Checkpoints/C3_local_reshapenet_g.pth')
                ...
                if cl_idx == 4:
                    ...
                    if least_lossC4 > val_epoch_loss:
                        ...
                        torch.save(local_model1.state_dict(),
                                   save_F + './Checkpoints/C4M1_localcheckpoint.pth')
                        torch.save(local_model2.state_dict(),
                                   save_F + './Checkpoints/C4M2_localcheckpoint.pth')
                        torch.save(local_model3.state_dict(),
                                   save_F + './Checkpoints/C4M3_localcheckpoint.pth')
                        torch.save(local_ViTmodel.state_dict(),
                                   save_F + './Checkpoints/C4_local_ViTmodel.pth')
                        torch.save(local_denoiser1.state_dict(),
                                   save_F + './Checkpoints/C4_denoiser1.pth')
                        torch.save(local_denoiser2.state_dict(),
                                   save_F + './Checkpoints/C4_denoiser2.pth')
                        torch.save(local_denoiser3.state_dict(),
                                   save_F + './Checkpoints/C4_denoiser3.pth')
                        torch.save(local_denoiser4.state_dict(),
                                   save_F + './Checkpoints/C4_denoiser4.pth')
                        torch.save(local_reshapenet.state_dict(),
                                   save_F + './Checkpoints/C4_local_reshapenet.pth')
                        torch.save(local_reshapenet_g.state_dict(),save_F + './Checkpoints/C4_local_reshapenet_g.pth')
                ...
                if cl_idx == 5:
                    ...
                    if least_lossC5 > val_epoch_loss:
                        ...
                        torch.save(local_model1.state_dict(),
                                   save_F + './Checkpoints/C5M1_localcheckpoint.pth')
                        torch.save(local_model2.state_dict(),
                                   save_F + './Checkpoints/C5M2_localcheckpoint.pth')
                        torch.save(local_model3.state_dict(),
                                   save_F + './Checkpoints/C5M3_localcheckpoint.pth')
                        torch.save(local_ViTmodel.state_dict(),
                                   save_F + './Checkpoints/C5_local_ViTmodel.pth')
                        torch.save(local_denoiser1.state_dict(),
                                   save_F + './Checkpoints/C5_denoiser1.pth')
                        torch.save(local_denoiser2.state_dict(),
                                   save_F + './Checkpoints/C5_denoiser2.pth')
                        torch.save(local_denoiser3.state_dict(),
                                   save_F + './Checkpoints/C5_denoiser3.pth')
                        torch.save(local_denoiser4.state_dict(),
                                   save_F + './Checkpoints/C5_denoiser4.pth')
                        torch.save(local_reshapenet.state_dict(),
                                   save_F + './Checkpoints/C5_local_reshapenet.pth')
                        torch.save(local_reshapenet_g.state_dict(),save_F + './Checkpoints/C5_local_reshapenet_g.pth')
        ...
        C1reshapenet_g = torch.load(save_F + './Checkpoints/C1_local_reshapenet_g.pth')
        C2reshapenet_g = torch.load(save_F + './Checkpoints/C2_local_reshapenet_g.pth')
        C3reshapenet_g = torch.load(save_F + './Checkpoints/C3_local_reshapenet_g.pth')
        C4reshapenet_g = torch.load(save_F + './Checkpoints/C4_local_reshapenet_g.pth')
        C5reshapenet_g = torch.load(save_F + './Checkpoints/C5_local_reshapenet_g.pth')
        ...
        C1reshapenet_g.update((x, y * D1) for x, y in C1reshapenet_g.items())
        C2reshapenet_g.update((x, y * D2) for x, y in C2reshapenet_g.items())
        C3reshapenet_g.update((x, y * D3) for x, y in C3reshapenet_g.items())
        C4reshapenet_g.update((x, y * D4) for x, y in C4reshapenet_g.items())
        C5reshapenet_g.update((x, y * D5) for x, y in C5reshapenet_g.items())
        ...
        # reshapenet_g weights
        reshape_g_dict = [C1reshapenet_g, C2reshapenet_g, C3reshapenet_g, C4reshapenet_g, C5reshapenet_g]
        reshape_g_weights.extend(reshape_g_dict)
        ...
        global_reshape_g_weights = fedAvg(reshape_g_weights)
        ...
        global_reshapenet_g.load_state_dict(global_reshape_g_weights)
            ...
            for epoch in range(args.val_global_ep):
                ...
                if cl_idx == 1:
                    g_val_epoch_loss, g_val_epoch_acc, g_val_epoch_iouwithback, g_val_epoch_iounoback, g_valepoch_iou_class0, g_valepoch_iou_class1 = eval(
                        val_loader, global_model1_fed, global_model2_fed, global_model3_fed, global_ViTmodel,
                        global_reshapenet, global_denoiser1, global_denoiser2, global_dn_pred1, global_dn_pred2,
                        loss_fn, folder)
