...
def eval(loader, local_model1, local_model2, local_model3, local_ViTmodel, reshapenet, vdenoiser1, vdenoiser2,
         global_dn_pred1, global_dn_pred2, loss_fn, folder):
...
def test(loader, modelclientFE, modelserver, modelclientBE, test_ViTmodel, reshapenet, tdenoiser1, tdenoiser2, loss_fn,
         folder):
    val_running_loss = 0.0
    seg_run_loss = 0.0
    recon1_run_loss = 0.0
    ...
    with torch.no_grad():
        for idx, (x, y) in enumerate(loader):
            ...
            targetsR = y.unsqueeze(1)
            targetsR = targetsR.repeat(1, 3, 1, 1)
            targetsR = targetsR.float()
            targetsR = F.interpolate(targetsR, size=(224, 224), mode="bilinear", align_corners=False)
            ViT_out = test_ViTmodel(targetsR).to(device)
            ...
            # combined_pred2 = normalize_features(predictions2) + normalize_features(ViT_out_noisy_resized)
            attention_weights2 = torch.sigmoid(torch.mean(predictions2, dim=(2, 3), keepdim=True))
            combined_pred2 = attention_weights2 * normalize_features(predictions2) + (1 - attention_weights2) * normalize_features(ViT_out_noisy)
