...
from utils_FBP_Blasto import (get_loaders, eval, get_loaders_test, test)
...
output_file = "/local-scratch/localhome/csj5/compressenv/output.txt"
...
#1. Train function
def train(train_loader, local_model1,local_model2,local_model3,local_ViTmodel,reshapenet,denoiser1,denoiser2,denoiser3,denoiser4,global_dn_pred1,global_dn_pred2,optimizer1,optimizer2,optimizer3, loss_fn):
    ...
    def visualize_features(features, title, ncols=5):
        num_samples = min(20, features.shape[0])  # Display at most 8 images
        nrows = (num_samples + ncols - 1) // ncols  # Calculate required rows    
        fig, axes = plt.subplots(nrows=nrows, ncols=ncols, figsize=(ncols * 3, nrows * 3))  # Adjust figure size    
        axes = axes.flatten()  # Flatten in case it's a 2D array        
        for i in range(num_samples):
            feature_map = features[i].mean(dim=0).detach().cpu().numpy()  # Average across channels
    ...
    recon_loss4_running = 0.0
    LDDM4_loss_running = 0.0
    all_predictions1=[]
    all_ViT_out = []
    ...
    for batch_idx, (data, targets) in enumerate(loop):
        ...
        all_ViT_out.append(ViT_out.detach().cpu())  
        ...
        all_ViT_out_noisy_resized.append(ViT_out_noisy_resized.detach().cpu())       
                
        combined_input1 = normalize_features(predictions1) + normalize_features(ViT_out_noisy_resized)
        attention_block1 = AttentionBlock(combined_input1.shape[1]).to(device)  
        combined_pred1 = attention_block1(combined_input1)
        all_combined_pred1.append(combined_pred1.detach().cpu())  
                        
        denoised_pred1 = denoise(combined_pred1, denoiser1, alpha_cum, t, device)
        ...
        combined_input2 = normalize_features(denoised_pred1) + normalize_features(global_dn_pred1)
        attention_block2 = AttentionBlock(combined_input2.shape[1]).to(device)  
        combined_denoised_pred1 = attention_block2(combined_input2)
        
        predictions2 = local_model2(combined_denoised_pred1)
        
        combined_input3 = normalize_features(predictions2)+ normalize_features(ViT_out_noisy_resized)
        attention_block3 = AttentionBlock(combined_input3.shape[1]).to(device)  
        combined_pred2 = attention_block3(combined_input3)                                                                                    
                                                   
        denoised_pred2 = denoise(combined_pred2, denoiser2, alpha_cum, t, device)
        recon_loss2 = F.mse_loss(normalize_features(denoised_pred2), normalize_features(predictions2))  
        
        combined_input4 =normalize_features(denoised_pred2)+ normalize_features(global_dn_pred2)
        attention_block4 = AttentionBlock(combined_input4.shape[1]).to(device)  
        combined_denoised_pred2 = attention_block4(combined_input4)     
        ...
        mygrad2 = grads2
        all_mygrad2.append(mygrad2.detach().cpu())  
        ...
        recon_loss4_running+= recon_loss4.item()
        all_denoised_pred4.append(denoised_pred4.detach().cpu())  
 
        predictions1.backward(denoised_pred4)
        optimizer1.step()
        optimizer1.zero_grad()
        loop.set_postfix(loss=loss.item())
    all_predictions1 = torch.cat(all_predictions1, dim=0)
    all_ViT_out = torch.cat(all_ViT_out, dim=0) 
    all_ViT_out_noisy_resized = torch.cat(all_ViT_out_noisy_resized, dim=0) 
    all_combined_pred1 = torch.cat(all_combined_pred1, dim=0) 
    all_denoised_pred1 = torch.cat(all_denoised_pred1, dim=0)
    all_mygrad2 = torch.cat(all_mygrad2, dim=0)
    all_denoised_pred4 = torch.cat(all_denoised_pred4, dim=0)       
    visualize_features(all_predictions1, "CS-FE model predictions")   
    visualize_features(all_combined_pred1, "noisy ViT features and CS-FE predictions")     
    visualize_features(all_denoised_pred1, "Denoised features from the LDDM-1")  
    visualize_features(all_mygrad2, "gradients from the server model")     
    visualize_features(all_denoised_pred4, "Denoised gradients from the LDDM-4")     
    epoch_recon_loss1 = recon_loss1_running / len(train_loader.dataset)
    print ("recon_loss1:",epoch_recon_loss1)

    epoch_LDDM1_loss = LDDM1_loss_running / len(train_loader.dataset)
    print ("LDDM1_loss:",epoch_LDDM1_loss)
    
    epoch_recon_loss4 = recon_loss4_running / len(train_loader.dataset)
    print ("recon_loss4:",epoch_recon_loss4)
...
def main():
    ...
    global_ViTmodel = models.vit_b_16().to(device)
    ...
    #global_denoiser3.load_state_dict(torch.load(save_F + './Checkpoints/global_denoiser2.pth'))   
    #global_denoiser4.load_state_dict(torch.load(save_F + './Checkpoints/global_denoiser1.pth')) 
    ...
    test_Acc,test_Iou_withback,test_Iou_noback,test_Loss,test_seg_loss, test_recon1_loss, test_recon2_loss =[],[],[],[],[],[],[]
    least_lossg = 100000000;
    least_losst=100000000;

    for com_round in (range(args.rounds)):
        local_weights1,local_weights2,local_weights3,ViT_weights, den1_weights,den2_weights,den3_weights,den4_weights,reshape_weights =[],[],[],[],[],[],[],[],[]
        ...
        for idx in idxs_users:
            ...
            for epoch in range(args.local_ep):
                ...
                if cl_idx == 1:
                    train_epoch_loss, train_epoch_acc, trainepoch_iou_withbackground, trainepoch_iou_nobackground, trainepoch_iou_class0, trainepoch_iou_class1, trainepoch_iou_class2, trainepoch_iou_class3, trainepoch_iou_class4 = train(train_loader, local_model1 , local_model2, local_model3,local_ViTmodel,local_reshapenet,local_denoiser1,local_denoiser2,local_denoiser3,local_denoiser4, global_dn_pred1,global_dn_pred2, optimizer1,optimizer2,optimizer3,loss_fn)
                    ...
                    val_epoch_loss, val_epoch_acc, valepoch_iou_withbackground, valepoch_iou_nobackground, valepoch_iou_class0, valepoch_iou_class1, valepoch_iou_class2, valepoch_iou_class3, valepoch_iou_class4 = eval(
                        val_loader, local_model1 , local_model2, local_model3,local_ViTmodel, local_reshapenet,local_denoiser1,local_denoiser2,global_dn_pred1,global_dn_pred2,loss_fn, folder)
                    ...
                    if least_lossC1 > val_epoch_loss:
                        ...
                        torch.save(local_ViTmodel.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C1_local_ViTmodel.pth')
                        torch.save(local_denoiser1.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C1_denoiser1.pth')
                        torch.save(local_denoiser2.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C1_denoiser2.pth')
                        torch.save(local_denoiser3.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C1_denoiser3.pth')
                        torch.save(local_denoiser4.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C1_denoiser4.pth')
                        torch.save(local_reshapenet.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C1_local_reshapenet.pth')                        
                ...
                if cl_idx == 2:
                    ...
                    if least_lossC2 > val_epoch_loss:
                        ...
                        torch.save(local_ViTmodel.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C2_local_ViTmodel.pth')
                        torch.save(local_denoiser1.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C2_denoiser1.pth')
                        torch.save(local_denoiser2.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C2_denoiser2.pth')
                        torch.save(local_denoiser3.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C2_denoiser3.pth')
                        torch.save(local_denoiser4.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C2_denoiser4.pth')
                        torch.save(local_reshapenet.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C2_local_reshapenet.pth')  
                ...
                if cl_idx == 3:
                    ...
                    if least_lossC3 > val_epoch_loss:
                        ...
                        torch.save(local_ViTmodel.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C3_local_ViTmodel.pth')
                        torch.save(local_denoiser1.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C3_denoiser1.pth')
                        torch.save(local_denoiser2.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C3_denoiser2.pth')
                        torch.save(local_denoiser3.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C3_denoiser3.pth')
                        torch.save(local_denoiser4.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C3_denoiser4.pth')                        
                        torch.save(local_reshapenet.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C3_local_reshapenet.pth')  
                ...
                if cl_idx == 4:
                    ...
                    if least_lossC4 > val_epoch_loss:
                        ...
                        torch.save(local_ViTmodel.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C4_local_ViTmodel.pth')
                        torch.save(local_denoiser1.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C4_denoiser1.pth')
                        torch.save(local_denoiser2.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C4_denoiser2.pth')
                        torch.save(local_denoiser3.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C4_denoiser3.pth')
                        torch.save(local_denoiser4.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C4_denoiser4.pth')
                        torch.save(local_reshapenet.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C4_local_reshapenet.pth')  
                ...
                if cl_idx == 5:
                    ...
                    if least_lossC5 > val_epoch_loss:
                        ...
                        torch.save(local_ViTmodel.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C5_local_ViTmodel.pth')
                        torch.save(local_denoiser1.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C5_denoiser1.pth')
                        torch.save(local_denoiser2.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C5_denoiser2.pth')
                        torch.save(local_denoiser3.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C5_denoiser3.pth')
                        torch.save(local_denoiser4.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C5_denoiser4.pth')
                        torch.save(local_reshapenet.state_dict(),'./Saved/baseline/Fed_Avg/Checkpoints/C5_local_reshapenet.pth')  
            ...
            for epoch in range(args.val_global_ep):
                ...
                if cl_idx == 1:
                    g_val_epoch_loss, g_val_epoch_acc, g_val_epoch_iouwithback, g_val_epoch_iounoback, g_valepoch_iou_class0, g_valepoch_iou_class1, g_valepoch_iou_class2, g_valepoch_iou_class3, g_valepoch_iou_class4 = eval(
                        val_loader, global_model1_fed, global_model2_fed, global_model3_fed,global_ViTmodel,global_reshapenet,global_denoiser1,global_denoiser2, global_dn_pred1,global_dn_pred2,loss_fn, folder)
...
# ------------------------------------------TESTING USING THE GLOBAL MODEL-----------------------------------------------------------------------
        ...
        for epoch in range(args.val_global_ep):
            ...
            test_epoch_loss, test_epoch_seg_loss,test_epoch_recon1_loss,test_epoch_recon2_loss, test_epoch_acc, test_epoch_accwithback,test_epoch_accnoback,global_dn_pred1,global_dn_pred2= test(test_loader,  M1_test, M2_test, M3_test,ViT_test,test_reshapenet,test_denoiser1,test_denoiser2, loss_fn, test_folder)
            print('\n')
            print(f"Testing dice loss: {test_epoch_loss:.3f}, Testing accuracy: {test_epoch_acc:.3f},Testing iou Score with background: {test_epoch_accwithback:.3f},Testing iou Score without background: {test_epoch_accnoback:.3f}")
